# Demo Sample  Implementation Guide

## Goal

Create a **safe, compilable DLL** that mimics the static characteristics of ransomware
so the `/predict` endpoint can be tested end-to-end with a real PE file:
...
The binary imports cryptographic and file-manipulation APIs (which EMBER2024 treats as
malicious signals) but contains **zero destructive logic**  functions are imported,
never called.
...
Save this as `demo/ransomguard_demo.c`:
 ...
 */
extern HANDLE WINAPI OpenProcess(DWORD dwDesiredAccess, BOOL bInheritHandle, DWORD dwProcessId);
...
static const unsigned char _entropy_pad[4096] = {
...
### Option A  Cross-compile on macOS / Linux (recommended)
...
x86_64-w64-mingw32-gcc \
    -shared \
    -o demo/ransomguard_demo.dll \
    demo/ransomguard_demo.c \
    -ladvapi32 -lkernel32 -lbcrypt \
    -Wl,--subsystem,windows
```

Verify the output is a valid PE file:

```bash
file demo/ransomguard_demo.dll
# Expected: PE32+ executable (DLL) ...
```

### Option B  Native Windows (MSVC)

```cmd
cl /LD ransomguard_demo.c /link advapi32.lib kernel32.lib bcrypt.lib
```
...
Expected response (model **not** loaded  service returns 503):

```json
{
    "detail": "Model not found at ./models/ember2024_malicious.txt. Run: python scripts/download_model.py"
}
```
...
The EMBER2024 feature extractor (`thrember`) analyses the PE file's:

| Feature         | Signal in this DLL                                                                                                                |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Shannon entropy | Elevated by `_entropy_pad` section (~7.8 bits/byte)                                                                               |
| Import table    | Contains `CryptEncrypt`, `BCryptEncrypt`, `CryptGenRandom`, `VirtualAlloc`, `CreateRemoteThread`, `RegSetValueExW`, `DeleteFileW` |
| Section count   | Standard (`.text`, `.rdata`, `.reloc`)  not anomalous on its own                                                                 |
| Export count    | 0 (no exported symbols  typical of packed/obfuscated malware)                                                                    |

These are **static** signals read from the PE header and import table.
The binary never executes any of the imported functions.
...
- [ ] Cryptographic APIs imported but **never called**
- [ ] `DllMain` body is intentionally empty
- [ ] Safe to scan with any AV (it may still flag it  that is expected and correct)
