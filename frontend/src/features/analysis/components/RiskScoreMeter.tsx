...
export function RiskScoreMeter({ score, verdict }: RiskScoreMeterProps) {
      ...
      <div className="relative h-32 w-32">
        <svg className="transform -rotate-90 h-32 w-32">
          ...
          <circle
            ...
            className={cn('transition-all duration-1000 ease-out', getRiskScoreColor(verdict))}
          />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className={cn('text-3xl font-bold', getRiskScoreColor(verdict))}>{score}</span>
